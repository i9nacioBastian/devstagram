...
@section('contenido')
    <div class="container mx-auto md:flex">
        <div class="md:w-1/2">
            ...
            <div class="">
                <p class="font-bold"><a href="{{ route('posts.index', $user->username) }}">{{ $post->user->username }}</a>
                </p>
                ...
                @if ($post->user_id === auth()->user()->id)
                    <form action="{{ route('posts.destroy', $post) }}" method="POST">
                        ...
                        <input type="submit" value="Eliminar Publicación"
                            class="bg-red-500 hover:bg-red-600 p-2 rounded text-white mt-4 font-bold cursor-pointer">
                    </form>
                @endif
            @endauth
        </div>
